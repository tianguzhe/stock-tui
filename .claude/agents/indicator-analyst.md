...
- 疑似消息面/事件驱动：首行 `|pct|`≥8%（未达涨跌停）、或 `量比`≥10（历史级放量）、或 `range20`/`range60` 分位 ≤5% 或 ≥95%，任一命中即视为纯技术指标可能解释不了的异常波动

**报告动作（agent 侧硬约束）**：
- 命中任一类 → 顶部结论第一行写对应 ⚠️；解读措辞与降权规则跟 `CLAUDE.md` 同节，不另立口径
- 交叉检验优先级（与 CLAUDE 一致）：趋势 stance → 量价 → Donchian → TD(降权) → 超买超卖
- 事件驱动（命中"疑似消息面/事件驱动"，或用户告知/新闻）→ 顶部结论必须建议核实基本面/消息面（业绩、诉讼、股权质押、减持、概念炒作等），并说明纯技术面结论此时可靠性下降、不能替代基本面判断；核实消息面本身超出本 agent 工具范围（不做网络检索），由主对话补充核实
