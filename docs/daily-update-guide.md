...
执行 `./scripts/daily-update.sh` 会自动完成：

1. **批量保存快照** - 保存所有股票当日的技术指标到 snapshot 表
2. **更新 RS 排名** - 计算相对强度百分位排名
3. **回填决策结果** - 回填 decision_log 中满10天的记录
4. **生成选股表** - 根据 .holdings 配置生成推荐列表（可选）
...
=== 2/4 更新 RS 相对强度排名 ===
rs-rank: updated 375 stocks

=== 3/4 回填决策结果 ===
backfill: 12 条已回填, 3 条跳过（数据不足）

=== 4/4 生成选股表 ===
持仓配置: sh601138:72.825:400,...
[选股表输出]
