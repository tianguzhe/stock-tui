...
>
> ✅ **三方交叉验证**（sh601208 东材科技 2026-07-24）：K 行 `['2026-07-24','41.48','41.72','43.17','40.30','546870.00',{},'5.41','229690.31','']`
> —— `row[5]`量 546870 = `qt[6]` = 东财 `f47`；`row[7]`换手 5.41 = `qt[38]` = 东财 `f168`(541)；
> `row[8]`成交额 229690.31万 = `qt[37]` = 东财 `f48`(2296903108 元)。三个源逐位一致，现有解析口径正确。
...
|------|------|------|----------|
| [1] | 简称 | — | Name |
| [7] | **外盘**（主动买） | 手 | `snapshot.outside_vol` |
| [8] | **内盘**（主动卖） | 手 | `snapshot.inside_vol` |
| [38] | 换手率 | % | （当日实时换手，序列换手仍取 `row[7]`） |
| [43] | 振幅 | % | 当日实时振幅（历史振幅仍靠本地算） |
| [46] | **市净率 PB** | 倍 | **不使用**（ETF 无 PB 返回 `0.00`） |
| [47] | 涨停价 | 元 | 不使用（= 昨收×1.1，用作字段定位锚点） |
| [48] | 跌停价 | 元 | 不使用（= 昨收×0.9） |
| [49] | **量比** | 倍 | `vol_ratio` 优先；`<=0` 回退本地 `Volume/MA20` |

> ⚠️ **[46] 是市净率不是量比**（2026-07-25 修正）。误取会让个股 `vol_ratio` 变成 PB（1~7 量级，远高于 `VolSurge`=1.5 / `VolStrong`=2.0），几乎所有个股被判成"放量"，系统性污染 `score.Volume`、`evalBullBear` 资金维度与 screener 的量比门槛；而 ETF 无 PB 返回 `0.00` 恰好触发本地回退、显示正常，使该错误只在个股上出现而长期隐蔽。
>
> **字段定位方法**：`[47]`/`[48]` 精确等于昨收 ×1.1 / ×0.9，以此锚定偏移量，再验证相邻字段。
>
> **东财独立交叉验证**（sh601208 东材科技 2026-07-24，东财价格类字段为 ×100 整数）：
>
> | 项 | 东财 push2 | 换算 | 腾讯 qt |
> |---|---|---|---|
> | 外盘 | `f49`=259823 | 259823 | `[7]`=259823 |
> | 内盘 | `f161`=287047 | 287047 | `[8]`=287047 |
> | 量比 | `f50`=77 | 0.77 | `[49]`=0.77 |
> | 市净率 | `f167`=698 | 6.98 | `[46]`=6.92 |
> | 换手率 | `f168`=541 | 5.41 | `[38]`=5.41 |
...
> ⚠️ **每行恒 11 列，`fields2` 勿加实时字段**：K 线接口只返回 K 线固有字段(f51–f61)，列数由 `fields2` 决定但**仅限这 11 个**。`f116`(总市值)/`f117`(流通市值)等属**实时行情 `push2/.../stock/get`** 接口，塞进本接口 `fields2` 会被**静默忽略**(返回仍 11 列)——若同时把解析门槛设成 `< 12` 会导致**每行都被跳过、东财 fallback 静默失效**(2026-07 曾踩此坑，`FetchEMKline`)。需要总市值请单独调 `FetchStockInfo`(读 `f116/f117`)，不要加进 kline `fields2`。
