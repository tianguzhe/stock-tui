...
- **SAR vs SuperTrend 止损口径(不同翻空节奏,日志/选股统一用此标准)**:`SAR`(抛物线、AF 0.02→0.20 紧贴价格)**翻空最早、最灵敏**,宜作移动止损;`SuperTrend`(ATR×3 通道、翻转慢)**滞后于 SAR**,宜作大趋势 stance 总览。**只盯 ST 止损会扛过整段下跌**(SAR 先报警、ST 慢承认)。正确用法:
  ...
  - ⚠️ **选股表的 `⚠️SAR翻空`/`⚠️SAR/ST双空` 只表示线位于现价上方,不表示当日翻转**——判当日翻转看 CLI 的 `reversed=true` 或近15日行的 `*`,误读会把「维持空头第 N 日」报成「今日翻空」
  - ⚠️ **SAR 刚翻转后距离必然过宽**:翻转时 AF 重置为 0.02、值取自前期极值,大涨日翻多可距现价 20%(涨停后必然翻空的镜像情形)。此时 `sar_value` 的 ÷ATR 名义合规但**无风控意义**,应改用 MA5/MA10 等贴身档并按新档重算敞口
...
- ⚠️ **对账单费用栏 `--` ＝ 尚未结算（约两日后补记），不是「无费用」**：2026-08 据此把 7.47 判为 0.00，两版后被实测推翻。**判据必须是结算后的截图**
- ⚠️ **App「参考盈亏」只扣已结算费用**——用它交叉验证市值法时，当日未落地的费用会造成系统性差额；**两个未结算量互证必然自洽却同时错**
- ⚠️ **排除法的结论只在其检验窗口内成立**：曾验证「持仓在 08-18~08-25 无除权」即断言无分红，而红利恰在 **08-25 入账、08-26 除权**，落在窗口外
- ⚠️ **残差排查不要默认单一成因**：+39.03 实为「漏记红利 +46.50」与「错记费用 −7.47」两个反向错误抵消后的残数——既不等于任何单笔，也不像典型利息金额，误导了三版排查方向
- 🟢 **券商成本价栏可反向验证费用推算**：买入后 `(旧股数×旧成本 + 买入净额) ÷ 新股数`、卖出后 `(加权成本额 − 卖出净额) ÷ 剩余股数`；复现值与 App 成本价栏逐位相同，即同时证明费用推算正确与「成本价含费用」
...
- ⚠️ **`backfill` 输出的分层胜率表不可直接引用**：`correct` 判据是 `pct > 0`（**绝对收益，不扣大盘**，`cmd/stockdb/main.go:283`），且样本高度不独立——实测 ⭐⭐⭐ 48 条中 **47 条挤在 9 个交易日、28 只标的**，前向窗口重叠，实质是「对一段行情的一次观察」而非 48 次试验
  - 引用前三步：① 用 `COUNT(DISTINCT log_date)`/`COUNT(DISTINCT code)` 看有效样本；② **与同期全池基准对照**（同法算全池 53.4%/+4.22%，⭐⭐⭐ 为 85.4%/+18.05%，**超额才是信号**）；③ 确认各层时间窗口一致——持仓层覆盖到 08-13、⭐⭐⭐ 停在 07-14（避开 7 月下跌），**直接相减无意义**
...
- ⚠️ **无 `open` 列**——只有 `low`/`high`/`close`/`amplitude`；要开盘价须走 `api.FetchDailyKline` 或腾讯 proxy，不能查库
...
- 🔴 **前复权基准漂移：补完历史日必须跟一次 `repair-scores --all`**
  ...
  - **免备份检测**（无需旧库、可覆盖全部行）：`LAG(close) OVER (PARTITION BY code ORDER BY trade_date)` 算出的环比涨幅与 `change_pct` 相减，差值即漂移量。⚠️ **它测的是「基准是否一致」，不是「有没有除权」**——全为 0 只能说明该期间**没有混合基准**，**不能推出无除权**。原因：若 close 是统一基准的前复权序列，除权日的历史价整体下移 D，`(close[i]−close[i−1])/close[i−1]` 与交易所口径的 `change_pct` **恒等**，drift 天然为 0。2026-08-25 曾据此误判「大唐 8 月无除权」，属方法误用
    - ⚠️ **该检测有伪阳性**：snapshot 逐日累积、股票池逐日变动，某只标的中间缺日时 `LAG` 会**跨日取值**，涨幅天然对不上。**须用交易日序号过滤**——`ROW_NUMBER() OVER (ORDER BY trade_date)` 之差为 1 才是相邻交易日。2026-08-27 全库统一后初查 **467 行「漂移」，过滤后真漂移 0 行**
...
- ⚠️ **`MinStopPct` 的对称另一端：止损太远同样会失效,而系统只对候选报警、对持仓不报**。高 ATR% 标的会出现「任何合规止损下都超预算」:实测 ATR% 6.88、120 日内 20% 的交易日单日波动 ≥8% 的标的,配能扛波动的宽止损(÷ATR 2.9)时合规仓位仅 **53 股,低于 1 手**;改配敞口合规的窄止损(÷ATR 1.35)则**平均 24 个交易日被击穿一次**。**此时纪律是在说「仓位应极小或为零」,不是纪律失灵——不要靠换止损口径把敞口算合规**(screener 对候选输出「止损距离过宽,建议观望」,但对已持仓标的只算敞口倍数,不会提示该倍数在任何合规仓位下都下不来)
- **加仓闸门 `🚫禁止加仓`**（`screener.AddOnBlocked`）：选股表持仓行「关键信号」列**首位**的操作禁令，排最前是因为跟在十几个信号后面等于没有。前置条件＝**浮亏 + SAR/ST 双空**，满足后由两条独立路径触发：score < `AddOnMinScore`=50，或浮亏 > `AddOnDeepLossPct`=20%（**深亏兜底，不看 score**，封住"score 达标但已深套"的尾部）。**趋势任一 stance 仍多则一律放行**——深度浮亏但 SAR/ST 回多是反转后补仓，不是摊平，拦掉会误伤。口径依据 2026-06~08 加仓审计：22 次加仓中 15 次为浮亏摊平（均值 −9.5%，最深 −21.8%），且资金投入最重时恰是系统报警最强时（score 23–44 + 双空）；回放 21 笔有效加仓拦 7 笔，覆盖最深的 6 笔摊平，零误伤。⚠️ `formatSignals` 同时被 `saveDecisions` 调用，故警示会一并写入 `decision_log.signals`（该字段无下游解析，回测读 snapshot 的 `sig_*` 布尔列），闸门触发历史可追溯
